...
export default async function OrderDetailPage({
  ...
  const waText = encodeURIComponent(
    `Hello Sujith, I am tracking my Z-Electronics order #${order.id.slice(0, 8)} (${formatPrice(order.total_amount)}). Current status: ${order.status}. Could you provide the latest dispatch update?`
  );
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8 min-h-[80vh]">
      ...
      <div className="space-y-8">
        {/* Header banner */}
        <div className="rounded-3xl border-2 bg-card p-6 sm:p-8 shadow-sm space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-6">
            ...
            <div className="sm:text-right space-y-1">
              <span className="text-xs text-muted-foreground block font-bold">Grand Total</span>
              <span className="text-3xl font-black text-primary block">
                {formatPrice(order.total_amount)}
              </span>
              <span
                className={`text-[11px] font-extrabold px-2 py-0.5 rounded-full inline-block ${
                  isPaid
            ...
            <div className="text-xs sm:text-sm space-y-1">
              <p className="font-extrabold">
                {isCompleted
                  ...
                  : isPaid
                  ? "Order Status: Payment Received & Silicon Verification"
                  : "Order Status: Placed & Awaiting Cash Collection"}
              </p>
              <p className="opacity-90 leading-relaxed text-xs">
                {isCompleted
                  ? "Proprietor Sujith has verified all hardware specifications, completed bench testing, and marked delivery as completed."
                  : isShipped
                  ? `Package is on the way via ${order.courier_name || "Express Courier"}. Tracking No: ${order.tracking_number || "Available shortly"}.`
                  : isPaid
                  ? "Payment confirmed. Components, sensors, and pin headers are being tested and packed."
                  : "Order recorded under Cash on Delivery. Please keep exact cash ready upon delivery, or click Pay Online to pay via UPI."}
              </p>
            </div>
          </div>

          {/* Visual 4-Step Fulfillment Stepper */}
          <div className="pt-2">
            <h3 className="text-xs font-extrabold uppercase tracking-wider text-muted-foreground mb-6">
              Live Fulfillment Progress
            </h3>

            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {/* Step 1: Placed */}
              <div className="flex flex-col items-center text-center p-3.5 rounded-2xl bg-muted/40 border border-border/70 shadow-xs">
                <div className="h-10 w-10 rounded-full bg-emerald-600 text-white flex items-center justify-center font-bold mb-2 shadow">
                  <Check className="h-5 w-5 stroke-[3]" />
                </div>
                <span className="text-xs font-extrabold text-foreground">1. Order Placed</span>
                <span className="text-[10px] text-muted-foreground mt-0.5">Recorded</span>
              </div>
              ...
              >
                <div
                  className={`h-10 w-10 rounded-full flex items-center justify-center font-bold mb-2 shadow ${
                    isPaid ? "bg-emerald-600 text-white" : "bg-amber-500 text-white animate-pulse"
                  }`}
                >
                  {isPaid ? <Check className="h-5 w-5 stroke-[3]" /> : <CreditCard className="h-5 w-5" />}
                </div>
                <span className="text-xs font-extrabold text-foreground">2. Payment</span>
                <span className="text-[10px] font-semibold mt-0.5 text-muted-foreground">
                  {isPaid ? "Received & Verified" : "Pending (COD)"}
                </span>
              </div>
              ...
              >
                <div
                  className={`h-10 w-10 rounded-full flex items-center justify-center font-bold mb-2 shadow ${
                    isShipped ? "bg-emerald-600 text-white" : "bg-muted text-muted-foreground"
                  }`}
                >
                  {isShipped ? <Check className="h-5 w-5 stroke-[3]" /> : <Truck className="h-5 w-5" />}
                </div>
                <span className="text-xs font-extrabold text-foreground">3. Dispatched</span>
                <span className="text-[10px] text-muted-foreground mt-0.5">
                  {isShipped ? order.courier_name || "In Transit" : "Preparing"}
                </span>
              </div>
              ...
              >
                ...
                >
                  {isCompleted ? <Check className="h-5 w-5 stroke-[3]" /> : <Package className="h-5 w-5" />}
                </div>
                <span className="text-xs font-extrabold text-foreground">4. Delivered</span>
                <span className="text-[10px] text-muted-foreground mt-0.5">
        ...
        {/* Courier Tracking Box (if tracking number provided) */}
        {order.tracking_number && (
          <div className="rounded-2xl border-2 border-primary/30 bg-primary/5 p-5 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="space-y-1">
              ...
              <p className="font-extrabold text-foreground text-sm flex items-center gap-2">
                ...
                Courier: <span>{order.courier_name || "Express Delivery"}</span>
              </p>
              <p className="text-xs text-muted-foreground font-mono">
                Tracking / AWB Number: <strong className="text-foreground">{order.tracking_number}</strong>
              </p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="rounded-2xl border bg-card p-5 space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Official electronic hardware distributor for engineering prototypes and robotics.
            </p>
          </div>
        </div>

        {/* Itemized Breakdown Table */}
        <div className="rounded-3xl border-2 bg-card p-6 sm:p-8 shadow-xs space-y-4">
          ...
          <div className="border-t pt-4 flex flex-col items-end gap-1.5">
            <div className="w-full sm:w-64 space-y-1.5 text-xs">
              ...
              <div className="flex justify-between text-muted-foreground font-medium">
                <span>Packaging & Quality Test:</span>
                <span className="text-emerald-600 font-bold">Free</span>
              </div>
              <div className="border-t pt-2 flex justify-between items-baseline font-bold">
                <span className="text-foreground text-sm font-extrabold">Total Amount:</span>
        ...
        <div className="rounded-2xl border bg-gradient-to-r from-emerald-500/10 via-emerald-500/5 to-transparent p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              Proprietor Sujith is available on WhatsApp to answer component questions or coordinate express delivery.
            </p>
          </div>
          <a
          ...
          >
            <Button className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs gap-2 shadow hover:-translate-y-0.5 active:translate-y-0.5 transition-all">
